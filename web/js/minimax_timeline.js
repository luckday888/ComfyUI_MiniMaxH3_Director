...
class MiniMaxH3DirectorEditor {
    ...
    buildDOM() {
        ...
        // 独立采样音频试听条（手动试听）。与视频 .bd-live-sample 面板分开：
        // 视频面板只在 v2v 系任务显示，而 t2v（文生视频）正是音频生成的主力场景
        ...
        liveAudio.innerHTML = `
            <div class="bd-live-audio-head">
                ...
                <span class="bd-meta" data-r="live-audio-meta" data-i18n="liveSample.audioIdle">开启「音频预览」后，采样中可点 ▶ 试听当前步生成的音频</span>
            </div>
     ...
     *  采样运行中切换时，正在跑的采样下一步即按新开关解码/推送（运行中开→即时
     *  生效、关→即时停止空耗算力）。fire-and-forget：预览为 best-effort，上报
     *  失败静默不打断 UI。非运行时上报也无害——下次运行 init 会用提交值重置 */
    pushPreviewState() {
     ...
     *  isImageBatch() 对 r2v/t2v/i2v/mixed 等视频批量任务也为真（director 模式为
     *  prompt_batch），但这些任务在主时间线上有 duration groups（usesBatchTimeline()
     *  为真），与单轨编辑器共用主试听面板，应当支持；真正排除的是纯图片组卡片
     *  （isImageBatch() 且 !usesBatchTimeline()） */
    canPreviewAudio() {
