...
| **Exposure anchor** | Companion to segment continuity, **on by default** (an "Exposure anchor" checkbox in the output panel; can be turned off). With continuity, each exported segment is fully model-generated, so exposure drifts in one direction from the pinned anchor and accumulates across segments — later segments get progressively brighter. After decoding each segment (before trimming the guided prefix), this gently pulls the free region's overall exposure (per-channel mean) back to the previous segment's ending level: a global brightness correction capped at ±8%, no pixel blending and no content change, so it cannot smear or ghost. The anchor is the previous segment's already-corrected export, propagated along the chain to break cumulative drift. Active only when continuity actually pins (non-first segment) |
