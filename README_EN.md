...
| **Segment continuity** | Off by default. For multi-segment `t2v` / `i2v` / `fl2v` / `r2v` / `v2v` / `rv2v`, pin the previous generated tail (motion + generated audio) into the next sample, then trim the prefix. Context frames: 5 / 22 / 39 / 56 — **recommended default: 22**. When on, each segment keeps the model's full generated tail (due to the 17k+5 frame grid, each guided segment runs ~0.5s longer than its UI nominal duration) so sentence-final syllables / fade-out are not clipped. **Thanks to [ComfyUI-H3-Motion-Context](https://github.com/NikoDemon80/ComfyUI-H3-Motion-Context) for the implementation approach** |
| **Exposure anchor** | Companion to segment continuity, **on by default** (an "Exposure anchor" checkbox in the output panel with an adjacent **strength slider** 0–20%, default 8%; both can be adjusted/turned off). With continuity, each exported segment is fully model-generated, so exposure drifts away from the guided start toward the model's preferred level (often brighter frame by frame); because each next segment pins from the previous one's latent, the drift accumulates down the chain and reads as later segments getting progressively brighter. After decoding each segment (before trimming the guided prefix), this gently pulls the free region's overall exposure (per-channel mean) back to **the end of that segment's own guided prefix** — the real handoff frames decoded from the previous segment's latents. It applies only a global brightness gain, no pixel blending and no content change, so it cannot smear or ghost. **The correction feeds back into the chain**: the next segment's *video* pin re-encodes the previous segment's corrected export pixels (instead of the uncorrected latent), which actually breaks the per-segment accumulation. Audio still pins from the uncorrected latent (exposure is visual, so audio is unaffected); the only extra cost is one ~22-frame video VAE encode per segment. The per-segment correction is capped by the strength slider (default ±8%), so no single frame jumps |
