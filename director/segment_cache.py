...
# ── 外接组（graph-wired i2v_groups / r2v_groups）身份指纹 ───────────────────
...
def _segment_identity_fingerprint(seg: SegmentPlan, plan: DirectorPlan) -> dict[str, Any]:
    ...
    if isinstance(witness, dict):
        # 仅存入本段自己的 group 记录，整条链刻意不并入：段按自身身份复用，
        # 改一个 group 不应使其他段失效（否则面板会把每段都报为 mismatch）。
...
def save_first_pass_cache(
    ...
    *,
    ...
    # SelfLift 原生低清 carry，同样按稳定 stem 落盘
    low_path = root / f"{stem}.pre.low.pt"
    try:
        cpu_latent = _av_latent_to_cpu(av_latent)
        _write_via_temp(latent_path, lambda p: torch.save(cpu_latent, p))
...
def load_first_pass_cache(
    ...
    try:
        ...
        if isinstance(stored, dict) and _reject_owner_mismatch(stored, seg):
            # 段属主不符（文件可能因段移位属于别的段）：硬拦截，优先于一切复用
            return None
        missing_external = (
...
def _inspect_external_group_cache(
    ...
    for idx in indices:
        ...
        # 缓存保存端按段稳定 id 落盘（segid_*）；旧版位置命名（seg_*）作回退
        seg_sid = (
            str(record.get("id") or "").strip()
            if isinstance(record, dict)
            else ""
        ) or f"p{int(idx):04d}"
        ext_stem = f"segid_{_sanitize_seg_id(seg_sid)}"
        legacy_stem = f"seg_{int(idx):04d}"
        meta_path = root / f"{ext_stem}.pre.meta.json"
        latent_path = root / f"{ext_stem}.pre.av.pt"
        if not meta_path.is_file() and (root / f"{legacy_stem}.pre.meta.json").is_file():
            meta_path = root / f"{legacy_stem}.pre.meta.json"
            latent_path = root / f"{legacy_stem}.pre.av.pt"
