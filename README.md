...
| **曝光锚定** | 段间引导配套开关，**默认开启**（输出面板「曝光锚定」勾选框，可手动关闭）。开启段间引导后，每段成片均为模型自由生成帧，曝光相对固定锚点会有同向小偏移并逐段累加，导致后段系统性偏亮。本功能在每段解码后、裁掉引导前缀前，把自由生成区的整体曝光（per-channel 均值）温和拉回上一段收尾水平——仅做 ±8% 限幅的全局亮度校正，不混合像素、不改画面内容，故不会花屏/重影；锚点用上一段校正后成片，沿链条传递以打断逐段漂移。仅在段间引导实际生效（非首段、已钉入上段）时起作用 |
