...
| **段间引导** | 默认关闭；多段 `t2v` / `i2v` / `fl2v` / `r2v` / `v2v` / `rv2v` 时可开启，将上一段生成结果的末尾运动（及生成音频）钉入下一段采样再裁掉前缀。上下文帧数：5 / 22 / 39 / 56，**默认推荐为 22**。开启后每段保留模型实际生成的完整尾部（受 17k+5 帧网格取整影响，每段约比 UI 标称时长多 0.5s），以保证句尾读音/收音不被裁掉。**感谢 [ComfyUI-H3-Motion-Context](https://github.com/NikoDemon80/ComfyUI-H3-Motion-Context) 提供的实现思路** |
| **曝光锚定** | 段间引导配套开关，**默认开启**（输出面板「曝光锚定」勾选框，旁有**校正强度滑块** 0–20%、默认 8%，可手动关闭/调整）。开启段间引导后，每段成片均为模型自由生成帧，曝光从引导起点向模型偏好漂移（常见为逐帧变亮），且漂移会沿「上段 latent → 下段 pin」的引导链逐段累积，观感上后段越来越亮。本功能在每段解码后、裁掉引导前缀前，把自由生成区的整体曝光（per-channel 均值）温和拉回**本段引导前缀末尾**（上段 latent 直传解码出的真实衔接帧）——仅做全局亮度增益、不混合像素、不改画面内容，不会花屏/重影。**校正会反馈回引导链**：下一段的视频 pin 用上段校正后的成片像素重新 VAE 编码（而非未校正 latent），从而真正阻断逐段累积（音频仍从未校正 latent 衔接、与亮度无关，不受影响；每段仅多一次约 22 帧的视频 VAE 编码，开销很小）。单段校正幅度受强度滑块限幅（默认 ±8%），单帧画面不会突变 |
